...
class FbAuthController with Helpers {
  ...
  Future<bool> signInAdmin(BuildContext context,
      {required String email, required String password}) async {
    try {
      UserCredential userCredential = await _firebaseAuth
          .signInWithEmailAndPassword(email: email, password: password);
  ...
  Future<bool> signIn(BuildContext context,
    ...
    /*  Stream<QuerySnapshot<Object?>> dataUserLogin = await FbFirestoreController()
      ...
      showSnackBar(
          context: context, content: 'تأكد من إتصالك بالانترنت', error: true);
      print('Exception: $e');
    }
    return false;
  }
